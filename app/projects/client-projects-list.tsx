...
export function ViewDetailsSpinner({ curveId }: { curveId: string }) {
  return (
    ...
    <div className="relative px-3 mt-16 md:mt-24">
      ...
      <div className="grid grid-cols-1 gap-x-6 gap-y-24 lg:grid-cols-2 lg:gap-y-0 xl:gap-x-12">
        {workItems.map((item, index) => {
          const isEven = index % 2 === 0; // left side in DOM for a grid is even (0, 2, 4...)
          // Stagger classes
          const staggerClass = index === 0 ? "lg:mt-0" : (isEven ? "lg:-mt-12" : "lg:mt-48");

          const overlayId = `project-${item.id}`;
              ...
              <div className="relative z-10 flex flex-col gap-6 lg:gap-8 group">
                ...
                >
                  ...
                  {hoveredOverlayId === overlayId && (
                    <div className="pointer-events-none absolute left-0 top-0 z-20 will-change-transform" style={{ transform: "translate3d(var(--cursor-x),var(--cursor-y),0) translate3d(-50%,-50%,0)" }}>
                      <ViewDetailsSpinner curveId={`work-list-curve-${item.id}`} />
                    </div>
                  )}
                </Link>
